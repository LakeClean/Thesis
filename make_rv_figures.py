...
def plot_rv_time(ID,fit_params=[],limits=[20,60],SB_type=1,
    ...
    if find_rv:
        ...
        for i,all_ID in enumerate(all_IDs):
            ...
            epoch_vsini1s.append(np.median(vsini1s[i][limits[0]:limits[1]]))
            epoch_vsini2s.append(np.median(vsini2s[i][limits[0]:limits[1]]))
        ...
        rv_path = f'/home/lakeclean/Documents/speciale/rv_data/rv_{ID}.txt'
        rv_names = ['jd','date','rv1','rv2','e_rv1','e_rv2','vbary','vhelio',
                    'vsini1','vsini2','ampl1','ampl2', 'const', 'gwidth', 'limbd']
        rv_dat = [epoch_jds,epoch_dates,epoch_rv1s,epoch_rv2s,
                  epoch_rv1_errs,epoch_rv2_errs,epoch_vbary,epoch_vhelio,
                  epoch_vsini1s, epoch_vsini2s, epoch_ampl1s, epoch_ampl2s,
                  epoch_consts, epoch_gwidths, epoch_limbds]
        rv_dict = {}
        ...
        df = pd.read_csv(f'/home/lakeclean/Documents/speciale/rv_data/rv_{ID}.txt')
        rv_names = ['jd','date','rv1','rv2','e_rv1','e_rv2','vbary','vhelio']
    ...
    if (len(fit_params)> 0) and (SB_type == 2):
        ...
        if make_table:
            values_to_add = [k1,k2,e,w,p,v01,v02,e_k1,e_k2,e_e,e_w,e_p,e_v01,e_v02]
            names_to_add = ['k1','k2','e','w','p','v01','v02','e_k1','e_k2','e_e','e_w','e_p','e_v01','e_v02']
            for v,n in zip(values_to_add,names_to_add):
                mt.add_value(v,n,ID)
