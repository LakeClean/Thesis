...
def analyse_power(ID,saving_data = True, plotting = True,
    ...
    for f_,p_ in zip(np.isnan(f),np.isnan(p_filt)):
        if f_:
            print(f_)
            input()
        if p_:
    ...
    #Fitting background following Aigrain 2004
    '''
    def test_func(x,A,B,C):
        return A / (1 + (x*B)**C)

    params = lmfit.Parameters()
    B_vals = [1e5, 1e2, 0,350, 70]
    A_vals = [100, 10, 1, 1]
    for j in range(3):
        params.add(f'A{j}',value=A_vals[j])
        params.add(f'B{j}',value=B_vals[j])
        #params.add(f'C{j}',value=2)


    fit = lmfit.minimize(sf.background_res, params, args=(f[f > 1],p_filt[f > 1]),
    ...
    values = []
    for j in range(3):
        values.append(fit.params[f'A{j}'].value)
        values.append(fit.params[f'B{j}'].value)
    '''
