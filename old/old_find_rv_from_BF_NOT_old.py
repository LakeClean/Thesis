...
def plot_rv_time(ID,fit_params=[],limits=[20,60],SB_type=1,
    ...
    if True:
        ...
        offset1s = np.zeros(shape= (len(files),no_orders))
        offset2s = np.zeros(shape= (len(files),no_orders))
        offsets = np.zeros(shape= (len(files),no_orders))
        vrad1s = np.zeros(shape= (len(files),no_orders))
        vrad2s= np.zeros(shape= (len(files),no_orders))
        ampl1s= np.zeros(shape= (len(files),no_orders))
        ampl2s= np.zeros(shape= (len(files),no_orders))
        vsini1s= np.zeros(shape= (len(files),no_orders))
        vsini2s= np.zeros(shape= (len(files),no_orders))
        gwidths= np.zeros(shape= (len(files),no_orders))
        limbds= np.zeros(shape= (len(files),no_orders))
        consts= np.zeros(shape= (len(files),no_orders))
        ...
        for i,date in enumerate(all_dates):
            ...
            date = all_dates[i]
            jds[i] = Time(date).jd - 2457000 #Correcting jd the way Frank likes. Something to do with Tess.
            ...
            for k in [0,20]:
                ...
                for IDline in IDlines[:-1]:
                    if IDline.split(',')[0][11:].strip(' ') == 'KIC12317678':
                        #only using the files chosen earlier in folder_dates
        ...
        if len(epoch_jds)>0:
            rv_path = f'/home/lakeclean/Documents/speciale/rv_data/NOT_old_LOWRES_{ID}.txt'
