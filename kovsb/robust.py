...
def mean(inputData, Cut=3.0,return_sigma=False):
	"""
...
def polyfit(inputX, inputY, order, iterMax=25):
	...
	if n < 1000:
		...
		nLeft   = numpy.long( n - nPerSeg[0]*nSeg )
		nPerSeg[numpy.long(nSeg/2)] = nPerSeg[numpy.long(nSeg/2)] + nLeft
		r       = numpy.zeros(nSeg)
		s       = numpy.zeros(nSeg)
		r[0]    = numpy.median(u[0: numpy.long( nPerSeg[0] ) ])
		s[0]    = numpy.median(v[0: numpy.long( nPerSeg[0] ) ])
		i2      = nPerSeg[0]-1
		for i in range(1,nSeg):
			i1   = numpy.long( i2 )
			i2   = i1 + numpy.long( nPerSeg[i] )
