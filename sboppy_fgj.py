...
def fit_rv_model(parameters, hjd, rv_measurements, unc=True):
...
def fit_radvel(hjd, rv_measurements, k=[1., 1.], e=0., w=0, p=1., t0=0., v0=0., show = False):
    '''
    Requires the rv_measurements to be array with four lists: rv1, rv2, unc1, unc2
    ...
    parameters.add('e',  value=e, min=0., max=1., vary=False)
    parameters.add('w',  value=w, min=0., max=360., vary=False)
    ...
    #print(parameters)
    #print(rv_measurements)
    #print(hjd)
    #print(fit_rv_model(parameters,hjd,rv_measurements))
    rv_model_fit = lmfit.minimize(fit_rv_model, parameters, args=(hjd, rv_measurements), ftol=1e-12, max_nfev=1000)
    print('here?')
    if (show == True):
...
if __name__ == '__main__':
    ...
    hjd_new = np.array( np.linspace( np.min(t), np.max(t), 1000 ) )  # Make 1000 new time points.
    hjd_new.sort()

    #phase     = np.remainder(t, p) / p
    #phase_new = np.remainder(hjd_new, p) / p

    phase     = np.array( np.mod(t, p) / p )
    phase_new = np.array( np.mod(hjd_new, p) / p )
    ...
    plt.xlim( 56450, 56550) 
    plt.ylim( -150, 50 )
    plt.xlabel( r'MJD [d]', size = 15) 
    plt.ylabel( r'Velocity [km/s]', size = 15 )
    plt.axhline( v0, ls = 'dashed', color='cyan', lw = 3, alpha = 0.6 )
    ...
    plt.axhline( 0.0, ls = 'dashed', color='cyan', lw = 3, alpha = 0.6 )
    plt.title( rvguess, size = 15 )
    plt.legend()
    ...
    rv_new1 = radial_velocity(hjd_new, k=k1, e=e, w=w,        p=p, t0=t0, v0=v0)
    rv_new2 = radial_velocity(hjd_new, k=k2, e=e, w=w + 180., p=p, t0=t0, v0=v0)
    tt = np.argsort( phase_new )
    plt.plot(phase_new[tt], rv_new1[tt], label = 'Model, Primary')
    plt.plot(phase_new[tt], rv_new2[tt], label = 'Model, Secondary')
    plt.axhline( v0, ls = 'dashed', color='cyan', lw = 3, alpha = 0.6 )
    plt.ylim( -150, 50 )
    plt.xlim( -0.1, 1.1 )
