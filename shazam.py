...
def FIES_caliber(filename):
  ...
  data = np.rec.fromarrays([np.column_stack([wave[0,i],data[0,i]]) for i in range(len(col_names))], names = list(col_names))
...
def crm(wl,nfl,iters=1,q=[99.0,99.9,99.99]):
  ...
  for ii in range(iters):
    ...
    #cosmic = np.array([n for n in range(len(nfl)) if nfl[n] < cut]) # Søren added brackets
...
def getfit_CCF(rvs,ccf,SB_type=1):
  '''
  Get radial velocity and projected rotational velocity
  from CCF by fitting a Gaussian and collecting the location of 
  the maximum and the width, respectively.
  ...
  if SB_type==2:
    ...
    fit = TRFLSQFitter()
    double_gauss = fit(m_init, rvs, ccf)
    return rvs, double_gauss(rvs)
  ...
  fit_g = fitting.LevMarLSQFitter()
  gauss = fit_g(g_init, rvs, ccf)
  return rvs, gauss(rvs)

'''
def getRV_SB2(rvs,ccf):
...
def smoothBF(vel,bf,sigma=2.0):
  '''
  Søren note: Appears to convolve the bf found with a gaussian
              smoothing it thus.
  :params:
    vel  : array, velocity in km/s
    bf   : array, the broadening function
